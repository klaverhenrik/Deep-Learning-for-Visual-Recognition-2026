...
*Code 4 – Isolating the weight scale effect over 10 layers. A scale of 1.5 explodes, 0.01 vanishes, and $1/\sqrt{D}$ (the Xavier principle) stays stable. This is the core intuition behind all principled initialisation schemes.*
...
These exercises are designed to help you prepare for the written exam. They cover calculation, code reading, and theoretical understanding — the same mix you can expect in the exam.

---

## Exercise 1 — Activation Function Gradients by Hand

The derivative of the sigmoid is $\sigma'(z) = \sigma(z)(1 - \sigma(z))$.

**(a)** What is the (approximate) value of $\sigma'(z)$ for at $z = 0$ and $z = 10$?

**(b)** A deep network has 10 sigmoid layers. Assume each layer's neurons are saturated with $\sigma'(z) \approx 0.01$. By what factor has the gradient shrunk by the time it reaches the first layer? Express your answer as a power of 10.

**(c)** ReLU has gradient 1 for positive inputs and 0 for negative inputs. Explain why a gradient of 0 for negative inputs (dead ReLU) is considered less catastrophic than sigmoid saturation, which also produces near-zero gradients.

**(d)** Leaky ReLU uses slope $\alpha = 0.01$ for negative inputs. Compute the gradient of Leaky ReLU at $z = -3$ and $z = 3$. How does this help compared to standard ReLU?

**(e)** A sigmoid layer receives inputs $\mathbf{x} = [0.8, 0.6]^T$. The upstream gradient is $\partial L / \partial z = -1$. Compute the gradients $\partial L / \partial x_1$ and $\partial L / \partial x_2$.

---

## Exercise 2 — The Zig-Zagging Problem

**(a)** Explain in your own words why sigmoid activations cause the zig-zagging problem during gradient descent.

**(b)** A simple linear layer that computes $\mathbf{w}^T\mathbf{x}$ receives inputs $\mathbf{x} = [0.7, 0.3]^T$ (both positive, as would come from a sigmoid layer). The upstream gradient is $\partial L / \partial z = -1$. What sign are the partial derivatives $\partial L / \partial w_1$ and $\partial L / \partial w_2$.

**(c)** If instead the inputs were $\mathbf{x} = [-0.5, 0.9]^T$ (zero-centred), what is different about the signs of the partial derivatives?

**(d)** Explain what "zig-zagging" means geometrically in terms of the update direction and the direction toward the optimum.
...
test_transform = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.500, 0.480, 0.450],
                         std=[0.240, 0.235, 0.250]),
])
```

**(a)** What is wrong with this code?

**(b)** Where do the values `[0.485, 0.456, 0.406]` and `[0.229, 0.224, 0.225]` typically come from?

**(c)** A student argues that normalisation is unnecessary because batch normalisation inside the network will fix any scaling issues anyway. Give one concrete reason why input normalisation is still beneficial even when the first layer is followed by batch normalisation.

**(d)** Raw ImageNet pixel values are in the range $[0, 255]$. After `transforms.ToTensor()`, they are scaled to $[0, 1]$. After `transforms.Normalize(mean=0.5, std=0.5)` (simplified single-channel example), what is the output range?

---

## Exercise 4 — Weight Initialisation

**(a)** All weights in a network are initialised to $w = 0.0$. Explain why this causes a problem called symmetry breaking failure, and why the network fails to learn.

**(b)** A layer uses Xavier initialisation with $n_\text{in} = 512$ input units and $n_\text{out} = 256$ output units. Xavier initialises weights from $\mathcal{N}(0, \sigma^2)$ where $\sigma^2 = 2/(n_\text{in} + n_\text{out})$. The same layer uses Kaiming initialisation for ReLU, where $\sigma^2 = 2/n_\text{in}$. Compute $\sigma$. Why is the factor 2 in the numerator needed for ReLU but not for tanh?

**(c)** You initialise weights with a very large standard deviation (e.g. $\sigma = 5.0$) in a network with tanh activations. Describe qualitatively what will happen to the activations in the first few layers, and what effect this will have on training.

---

## Exercise 5 — Batch Normalisation

**(a)** After normalisation, batch norm applies $y_i = \gamma \hat{z}_i + \beta$ with learnable parameters $\gamma$ and $\beta$. At initialisation, $\gamma = 1$ and $\beta = 0$. What is the output $y_i$ at this point? What does this tell you about the role of $\gamma$ and $\beta$ during early training?

**(b)** Explain why batch norm uses running averages of mean and variance during inference rather than computing batch statistics.

**(c)** Batch norm is usually placed between the linear/conv layer and the activation function. A student places it after the ReLU. What practical consequence might this have on the distribution that batch norm sees?

---

## Exercise 6 — Reading PyTorch Code
...
**(a)** Why is `bias=False` passed to `nn.Conv2d`? Would it make a difference if you set `bias=True`?

**(b)** The block follows the Conv → BN → ReLU pattern. What two things would go wrong if you changed the order to Conv → ReLU → BN?

**(c)** `nn.ReLU(inplace=True)` modifies the input tensor in place rather than creating a new one. What is the benefit of this, and is there any risk?

**(d)** A student builds a 20-layer network entirely from `ConvBlock` modules but forgets to apply Kaiming initialisation. PyTorch's default initialisation for `nn.Conv2d` is Kaiming uniform. Will training still work reasonably? Explain.

---

## Exercise 7 — Theoretical: Putting It All Together

**(a)** A colleague trains a CNN and observes that the training loss decreases very slowly for the first 10 epochs, then suddenly improves. They are not using batch normalisation. Suggest two possible causes related to this lecture's topics and one diagnostic step for each.

**(b)** Batch normalisation is described as making training less sensitive to the choice of weight initialisation. Explain intuitively why this is the case.

**(c)** Batch normalisation adds learnable parameters $\gamma$ and $\beta$ per feature. For a convolutional layer with 128 output channels followed by `nn.BatchNorm2d(128)`, how many extra learnable parameters does batch norm add? Is this a significant overhead?
